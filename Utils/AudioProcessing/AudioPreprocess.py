...
def build_from_path(hparams, input_dirs, mel_dir, linear_dir, wav_dir, n_jobs=12, tqdm=lambda x: x):
    ...
    for input_dir in input_dirs:
        with open(os.path.join(input_dir, 'transcript_tts'), encoding='utf-8') as f: ### read input audio's file name and text to f variable
            i=0
            for line in f:
                parts = line.strip().split('|')
                wav_name = parts[0].strip().split('/')[1]
                wav_path = os.path.join(input_dir, parts[0])
                text = parts[2]
...
def main():
    ...
    parser.add_argument('--base_dir', default= 'data') ### because runing from current folder, need to move out before loading data
