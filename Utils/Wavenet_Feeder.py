...
from Utils.Utils import is_scalar_input
...
class Feeder:
    ...
    def __init__(self, coordinator, metadata_filename, base_dir, hparams):
        ...
        print('Debug: wavenet metadata dir {}'.format(self._data_dir))## debug
        with open(metadata_filename, 'r') as f:
            self._metadata = [line.strip().split('|') for line in f]
            if len(self._metadata)<=0:
                raise Exception('Empty training data (map.txt). Make sure Tacotron synthesis successfully executed before running Wavenet')

        print('Debug: length of metadata {}'.format(len(self._metadata)))  ## debug
        # Train test split
        ...
        print('Debug: test size {}'.format(test_size))  ## debug
        ...
        print('Debug: new test size {}'.format(test_indices))  ## debug
        print('Debug: train size {}'.format(train_indices))  ## debug
        self._train_meta = list(np.array(self._metadata)[train_indices])
        self._test_meta = list(np.array(self._metadata)[test_indices])

        self.test_steps = len(self._test_meta) // hparams.wavenet_batch_size
        print('Debug: test steps {}'.format(self.test_steps))  ## debug
        if hparams.wavenet_test_size is None:
        ...
        with tf.device('/cpu:0'):
            ...
            ## is raw or mulaw ==> scalar input ==> shape of audio input:(None,1,None), audio_target type: tf.float32
            ##is mulaw_quantizie ==> non scalar input ==> shape of audio input: (None, quantizie size,None), audio_target_type: tf.int32
            ...
            print('Debug: audio input place holder shape {}'.format(input_placeholder.shape))  ## debug
            print('Debug: audio target place holder shape {}'.format(target_placeholder.shape))  ## debug
            queue_types = [tf.float32, target_type, tf.int32] ### create a shape of queue with data type
            if self.local_condition:
                self._placeholders.append(
                    tf.placeholder(tf.float32, shape=(None, hparams.num_mels, None), name='local_condition_features'))
            ...
            print('Debug: queue types {}'.format(queue_types))  ## debug
